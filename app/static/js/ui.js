...
(function () {
  ...
  function initSearch() {
    ...
    const panel = document.getElementById('topbarSearchResults');
    if (!input || !panel) return;

    let temporizador = null;
    const ICONOS = {
      Tarea: '📋', Materia: '📚', Grupo: '👥', Recurso: '📎', Usuario: '👤'
    };
    ...
    function renderizar(resultados) {
      ...
      panel.innerHTML = resultados.map(function (r) {
        return '<a class="search-result-item" href="' + r.url + '">' +
          ...
          '<span class="search-result-body">' +
            ...
            '<span class="search-result-sub">' + r.tipo + ' · ' + r.subtitulo + '</span>' +
          '</span></a>';
      }).join('');
      panel.classList.remove('is-hidden');
    }
    ...
    input.addEventListener('input', function () {
      ...
      temporizador = setTimeout(function () {
        fetch('/buscar?q=' + encodeURIComponent(texto))
          ...
          .then(function (data) { renderizar(data.resultados || []); })
          .catch(function () { panel.classList.add('is-hidden'); });
      }, 250); // debounce
    });
  ...
  function initAccountMenu() {
    ...
    document.addEventListener('keydown', function (ev) {
      if (ev.key === 'Escape') menu.classList.add('is-hidden');
    });
  }
